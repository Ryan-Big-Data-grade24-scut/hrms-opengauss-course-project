# Course Materials Index

这份索引把和课程要求直接相关、以及你们组内已有的核心项目材料集中说明。

## 1. 项目输入材料

位于：

- [project_inputs](E:/Ufolder/Current/ActionSys/Hgclass/DB/docs/course_materials/project_inputs)

### [企业人力管理系统.docx](E:/Ufolder/Current/ActionSys/Hgclass/DB/docs/course_materials/project_inputs/企业人力管理系统.docx)

作用：

- 你们已有的人力管理系统项目草案
- 适合作为课程报告、模块拆分、系统说明的原始素材

### [人力管理系统设计文档.pdf](E:/Ufolder/Current/ActionSys/Hgclass/DB/docs/course_materials/project_inputs/人力管理系统设计文档.pdf)

作用：

- 现有设计说明沉淀
- 可用于对照当前数据库、前后端实现是否偏题

### [Backup_Recovery_System_Architecture.md](E:/Ufolder/Current/ActionSys/Hgclass/DB/docs/course_materials/project_inputs/Backup_Recovery_System_Architecture.md)

作用：

- 你们最初“备份恢复”方向的架构沉淀
- 现在更适合作为亮点模块和扩展方向参考

### [memory.md](E:/Ufolder/Current/ActionSys/Hgclass/DB/docs/course_materials/project_inputs/memory.md)

作用：

- 组内讨论上下文和想法记录
- 适合回看技术边界、老师口径、组员分工倾向

## 2. 正式课程要求

位于：

- [course_requirements](E:/Ufolder/Current/ActionSys/Hgclass/DB/docs/course_requirements)

其中最关键的是：

- [Database Course Design Requirements.docx](E:/Ufolder/Current/ActionSys/Hgclass/DB/docs/course_requirements/Database%20Course%20Design%20Requirements.docx)

它决定了：

- 课程硬要求是什么
- openGauss 是否必须
- 你们最终提交物应该包含什么

## 3. 当前工程输出

### 规划与规范

- [AGENT.md](E:/Ufolder/Current/ActionSys/Hgclass/DB/AGENT.md)
- [ACTIVE_PLAN.md](E:/Ufolder/Current/ActionSys/Hgclass/DB/master/ACTIVE_PLAN.md)
- [PROJECT_ROADMAP.md](E:/Ufolder/Current/ActionSys/Hgclass/DB/master/PROJECT_ROADMAP.md)
- [API_SPEC.md](E:/Ufolder/Current/ActionSys/Hgclass/DB/master/API_SPEC.md)
- [DATABASE_SPEC.md](E:/Ufolder/Current/ActionSys/Hgclass/DB/master/DATABASE_SPEC.md)

### 数据库

- [10_hrms_schema.sql](E:/Ufolder/Current/ActionSys/Hgclass/DB/sql/10_hrms_schema.sql)
- [20_sample_queries.sql](E:/Ufolder/Current/ActionSys/Hgclass/DB/sql/20_sample_queries.sql)

### 后端

- [backend](E:/Ufolder/Current/ActionSys/Hgclass/DB/backend)

### 前端

- [frontend](E:/Ufolder/Current/ActionSys/Hgclass/DB/frontend)

### 启动与操作手册

- [STARTUP_GUIDE.md](E:/Ufolder/Current/ActionSys/Hgclass/DB/docs/operations/STARTUP_GUIDE.md)

### 下一阶段路线图

- [NEXT_PHASE_ROADMAP.md](E:/Ufolder/Current/ActionSys/Hgclass/DB/master/plans/NEXT_PHASE_ROADMAP.md)
