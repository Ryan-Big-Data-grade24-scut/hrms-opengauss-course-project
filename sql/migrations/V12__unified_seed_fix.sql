-- V12: 统一种子数据修复
-- 修复 V1-V11 迁移之间的数据冲突，确保从零启动有完整数据
...
TRUNCATE TABLE approval_request CASCADE;
TRUNCATE TABLE approval_step CASCADE;
...
(1, 'CEO', 1), (2, 'VP Engineering', 1), (3, 'Engineering Manager', 3),
(4, 'Senior Backend Engineer', 2), (5, 'Backend Engineer', 4),
(6, 'Senior Frontend Engineer', 2), (7, 'Frontend Engineer', 3),
(8, 'DevOps Engineer', 3), (9, 'QA Engineer', 3),
(10, 'VP Product', 1), (11, 'Product Manager', 5), (12, 'UX Designer', 4),
(13, 'Data Analyst', 4), (14, 'VP Sales & Marketing', 1),
(15, 'Sales Representative', 6), (16, 'Marketing Specialist', 5),
(17, 'Customer Success Manager', 3), (18, 'VP Operations', 1),
(19, 'HR Specialist', 3), (20, 'Accountant', 3);
...
(519,'NT0017','Kevin He','M','2022-04-01','active',1,8,37,60,0,0,0,505),
...
-- 补联系方式
UPDATE employee SET phone = '13' || LPAD(CAST(70000000 + employee_id * 7 % 10000000 AS TEXT), 8, '0'), email = LOWER(REPLACE(REPLACE(full_name, ' ', '.'), '''', '')) || '@hrms.com' WHERE phone IS NULL;
...
UPDATE employee SET phone = '13800138003', email = 'jack.yang@hrms.com' WHERE employee_id = 509;
...
SELECT e.employee_id, d::date,
    (d::date + time '09:00' + (random() * interval '30 minutes'))::timestamp,
    (d::date + time '18:00' + (random() * interval '60 minutes'))::timestamp,
    CASE WHEN random() < 0.7 THEN 'present' WHEN random() < 0.85 THEN 'late' WHEN random() < 0.95 THEN 'absent' ELSE 'half-day' END
FROM employee e, generate_series('2025-01-01'::date, '2026-05-31'::date, '7 days'::interval) d
WHERE e.employment_status IN ('active','probation') AND e.employee_id NOT IN (SELECT employee_id FROM employee WHERE employee_id > 562)
AND NOT EXISTS (SELECT 1 FROM attendance_record r WHERE r.employee_id = e.employee_id AND r.record_date = d::date);

-- ===================================================================
-- 7. 绩效数据（每员工2期）
-- ===================================================================
INSERT INTO performance_review (employee_id, reviewer_id, review_period, rating, score, status)
SELECT e.employee_id, COALESCE(e.manager_employee_id, e.employee_id), p,
    CASE WHEN e.employee_id % 7 = 0 THEN 2 WHEN e.employee_id % 5 = 0 THEN 4 ELSE 3 END,
    CASE WHEN e.employee_id % 7 = 0 THEN 55 + e.employee_id % 20
         WHEN e.employee_id % 5 = 0 THEN 75 + e.employee_id % 15
         ELSE 60 + e.employee_id % 25 END,
    'submitted'
FROM employee e, (VALUES ('2025-Q4'), ('2026-Q1')) AS p(period)
WHERE e.employment_status IN ('active','probation') AND e.employee_id <= 562
AND NOT EXISTS (SELECT 1 FROM performance_review r WHERE r.employee_id = e.employee_id AND r.review_period = p.period);

-- ===================================================================
-- 8. 高离职风险员工额外异常数据
-- ===================================================================
UPDATE employee SET engagement_score = 45, attendance_absent_count = 8, attendance_late_count = 12 WHERE employee_id = 540;
UPDATE employee SET engagement_score = 53, attendance_late_count = 10 WHERE employee_id = 530;
UPDATE employee SET attendance_late_count = 8 WHERE employee_id = 553;

SELECT 'V12: unified seed data applied' AS result;
